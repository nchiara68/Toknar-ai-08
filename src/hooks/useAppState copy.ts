...
export const useAppState = (username: string, userEmail: string) => {
  ...
  // Handle file deletion - Complete S3 + DynamoDB cleanup
  const handleDeleteFile = useCallback(async (document: DocumentType) => {
    // Use confirm for user confirmation (this is secure)
    if (!confirm(`Delete "${document.name}" and all its chunks?\n\nThis action cannot be undone.`)) return;
    
    try {
      // Step 1: Try to delete the actual file from S3 bucket (non-blocking)
      ...
      // Still try to refresh the UI to see current state
      forceRefresh();
    }
